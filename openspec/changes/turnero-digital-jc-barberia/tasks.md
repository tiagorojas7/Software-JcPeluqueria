...
- [ ] 3b.7 GREEN: `ActorContext` + estrechamiento `WHERE barber_id = :actorBarberId` en el repositorio.
- [ ] 3b.8 RED **(matriz de amenazas)**: barbero pidiendo facturación del local → `403`. → *access-control: El barbero queda acotado a sus propios datos*
- [ ] 3b.9 GREEN: aplicar el mismo guard/estrechamiento a los endpoints de facturación.
- [ ] 3b.10 RED: test de contrato ruta × rol — matriz completa permitido/denegado por cada ruta declarada y cada rol. → *access-control: Matriz de permisos por rol*
- [ ] 3b.11 GREEN: completar decoradores faltantes hasta que 3b.10 esté en verde.
- [ ] 3b.12 RED: permisos de secretaria se amplían por fila de `role_permissions`, sin cambio de código. → *access-control: Permisos de secretaria ajustables sin cambio de código*
- [ ] 3b.13 GREEN: confirmar que el guard lee `role_permissions` en cada request, no un mapa hardcodeado.

Requisitos que cierra: **access-control** (4/6 restantes: Tres roles con aplicación en el backend · Matriz de permisos por rol · El barbero queda acotado a sus propios datos · Permisos de secretaria ajustables).
...
- [ ] 5.5 RED: checkout crea preference de MercadoPago por el 50% del precio de lista. → *client-booking: Reserva web con seña obligatoria del 50%*
- [ ] 5.6 GREEN: `MercadoPagoPaymentAdapter.createPreference()` + `CheckoutUseCase`.
- [ ] 5.7 RED **(matriz de amenazas)**: webhook con firma HMAC inválida → `401`, cero efectos en el dominio.
- [ ] 5.8 GREEN: verificar `x-signature` según lo confirmado en 5.1.
- [ ] 5.9 RED: webhook responde `200` de inmediato y encola `payment.process`; el worker consulta `GET /v1/payments/:id` como fuente de verdad, no el redirect.
- [ ] 5.10 GREEN: handler del webhook + job `payment.process`.
- [ ] 5.11 RED **(matriz de amenazas)**: reintento del mismo `payment_id` afecta cero filas.
- [ ] 5.12 GREEN: confirmar idempotencia en `ConfirmHoldOnPayment`.
- [ ] 5.13 RED **(matriz de amenazas)**: payload `approved` falsificado sin firma válida es rechazado.
- [ ] 5.14 GREEN: endurecer verificación de firma hasta que 5.13 pase.
...
- [ ] 7.6 GREEN: confirmar vía inyección de dependencias (token de puerto, no import directo).
