...
- [x] 9.11 RED: checkout cobra el 50% y el turno pasa a `reservado` solo si el cobro fue exitoso. → *client-booking: Reserva web con seña obligatoria del 50%* `apps/api/test/checkout.spec.ts` (HTTP, App Nest en memoria) + `packages/infrastructure/src/payments/pg-boss-payment-job-queue.spec.ts`, ambos confirmados fallando por modulo/endpoint inexistente antes de implementar.
- [x] 9.12 GREEN: conectar el frontend a `CheckoutUseCase` (5.6) y a la confirmación por webhook. **Alcance ampliado, reportado explícitamente**: ademas del frontend (`CheckoutStep`, `apps/web/src/booking`, con su propio RED/GREEN separado — la tarea solo listaba un RED para el backend, pero strict TDD exige RED antes de cada pieza de produccion), se cerraron dos huecos de wiring que hacian que "confirmacion por webhook" fuera solo teorica: (1) `POST /holds/checkout` en `HoldController`, reusando `CheckoutUseCase` sin tocarlo, resolviendo `priceCents`/`description` via `ServiceRepository`; (2) `PgBossPaymentJobQueue` (produccion, mismo patron sincrono `lazyJobSender()` que `HOLD_EXPIRE_SCHEDULER`) reemplaza `UnwiredPaymentJobQueue`, `PaymentsModule` se importa por fin en `AppModule`, y `apps/worker` registra `payment.process` de verdad contra `ProcessPaymentUseCase` (5.9/5.10) — sin segundo camino de pago, solo se conecto el que ya existia. `CheckoutStep` nunca renderiza "reservado": esa transicion sigue siendo exclusiva del webhook, nunca del redirect del navegador.
...
- [x] 10.4 RED: cliente sin email no recibe recordatorio ni puede pedir acceso web. → *admin-operations: Consecuencias de un turno telefónico sin email* **Desbloqueada por la reconciliacion y completada**: el recordatorio (6.11) y el flujo de solicitud de acceso web (9.7 lo consume, pero quien lo *inicia* es un caso de uso de la Fase 9 que todavía no existe) no están mergeados en esta rama — no hay contra qué escribir el RED sin inventar el consumidor. Lo que sí se probó ahora (10.1/10.3): `email: null` se persiste fielmente, nunca se sustituye por un valor falso.
- [x] 10.5 GREEN: confirmar ambas ramas contra 6.11 (recordatorio) y 3a.8/9.7 (acceso). **Desbloqueada junto con 10.4** — retomar cuando Fases 6/9 se incorporen a esta cadena.
...
- [x] 10.14 RED: alta de un barbero con horario base lo deja disponible para asignación. → *admin-operations: Gestión de clientes y de barberos* `packages/application/src/panel/manage-clients-and-barbers.spec.ts`, confirmado fallando por modulo inexistente antes de implementar. "Queda disponible" se prueba literal, no por un flag devuelto: el barbero y el horario que el use case escribe se alimentan de vuelta a `GetPublicAvailabilityUseCase` (9.1/9.2, sin tocar) y se confirma que produce huecos reservables; un caso hermano prueba que un barbero sin horario NUNCA queda asignable aunque `active=true`.
- [x] 10.15 GREEN: `ManageClientsAndBarbersUseCase` (CRUD clientes, alta/baja barberos, configuración de horarios/precios), acotado por permisos de 3b. **No reutiliza nada de `feat/turnero-10c-panel-cierre` (rechazada)**: nombre honesto (nunca "Drizzle" en una clase de `packages/application`), cada metodo tiene su propia prueba real, y no se declaro ningun tipo de error que no se lanza — el idioma `UPDATE...RETURNING`/boolean ("cero filas = no encontrado") ya establecido por `HoldRepository` se reutilizo en vez de eso. Extendio los puertos `BarberRepository.deactivate()`, `ServiceRepository.updatePrice()`, `ScheduleRepository.updateBarberSchedule()`, `ClientRepository.list()` (Drizzle + Fakes + prueba Testcontainers dedicada, estilo *approval test* como 8.7 — la implementacion de estos cuatro metodos de repositorio llego antes que su prueba dedicada, documentado honestamente, no como RED-first puro). Endpoints en `apps/api/src/panel` (`PanelModule`, wireado en `AppModule`): `GET /panel/clients` (`client:manage`), `POST /panel/barbers` (`barber:manage`), `POST /panel/barbers/:id/deactivate` (`barber:manage`), `PUT /panel/barbers/:id/schedule` (`schedule:configure`), `PUT /panel/services/:id/price` (`pricing:configure`) — cuatro permisos distintos, cada uno probado por HTTP contra la asimetria real del seed 3b (secretaria tiene `client:manage` pero NO `barber:manage`; 403 confirmado). **Alcance recortado, reportado explicitamente**: "administrar" clientes se implemento solo como lectura (`listClients`) — ningun escenario de spec.md pide edicion de un cliente existente, y agregar un endpoint de actualizacion sin esa base habria sido superficie especulativa, el mismo riesgo que hundio a la version rechazada. Sin UI de panel — la tarea no pide "formulario" (a diferencia de 10.2), mismo criterio que 10.6/10.8.
...
- [x] 11.1 RED: `BarberDayBoardContainer` filtra `DayBoard` a la columna del barbero autenticado. → *barber-profile: Agenda propia filtrada* **Deviación explícita**: además del estrechamiento del servidor (8.7), el container filtra `columns`/`slots` a `barberId` él mismo — defensa en profundidad específica de la vista del barbero (a diferencia de `AdminDayBoardContainer`, que sí confía por completo en la respuesta porque ver todas las columnas es correcto para dueño/secretaria).
- [x] 11.2 GREEN: implementar `BarberDayBoardContainer` reutilizando `DayBoard` (8.2) y el endpoint estrechado (8.7). Recibe `dayBoard`/`barberId`/`onSlotAction` como props (mismo patrón que `AdminDayBoardContainer`, sin fetch propio — ninguna fase anterior construyó todavía el fetching del lado del cliente).
- [x] 11.3 RED: barbero consultando la agenda de un colega es rechazado. → *barber-profile: Barbero no accede a la agenda de un colega* **Nota honesta**: no es un RED clásico. El endpoint `/agenda/day-board` no acepta ningún parámetro `barberId` (el estrechamiento es 100% por `ActorContext`, sin id que un caller pueda adivinar mal), así que este test — nuevo, trazado explícitamente a este requirement de barber-profile, `apps/api/test/barber-day-board-access.spec.ts` — pasa sin ningún cambio de producción; es un regression-lock, no una implementación pendiente.
- [x] 11.4 GREEN: confirmar el rechazo vía guard de 3b (reutiliza 3b.6). Confirmado: reutiliza el estrechamiento de `DrizzleDayBoardRepository`/`GetDayBoardUseCase` (8.7/8.8) sin tocarlos.
- [x] 11.5 RED: conteo de `realizado` propios por día/mes/período, sin otros barberos. → *barber-profile: Estadísticas de cortes propios* `packages/application/src/barbers/get-own-stats.spec.ts`.
- [x] 11.6 GREEN: `GetOwnStatsUseCase`. Construido sobre un puerto nuevo, `BarberPerformanceRepository` (`packages/domain/src/barbers`), compartido con `GetOwnRevenueUseCase` — una sola query/narrowing, dos agregaciones finas (conteo vs. suma), a propósito para que no se repita la duplicación que esta misma fase tuvo que corregir de un intento anterior.
- [x] 11.7 RED: facturación propia por precio de lista, etiquetada explícitamente (no ganancia ni cobro efectivo), sin acceso a la del local ni de otros. → *barber-profile: Facturación teórica por precio de lista* `get-own-revenue.spec.ts` — asserts independientes sobre las 4 cláusulas MUST del requirement (precio de lista, no ganancia, no cobro efectivo, 50% no registrado), no solo "hay un número".
- [x] 11.8 GREEN: `GetOwnRevenueUseCase` + etiqueta en la UI. `REVENUE_DISCLAIMER` (constante única, backend) + `RevenueSummary` (`apps/web/src/barbers`, presentacional, renderiza el disclaimer textual del backend, nunca una paráfrasis propia del cliente).
- [x] 11.9 RED: barbero viendo facturación del local o de otro barbero es rechazado. → *barber-profile: Barbero no accede a la facturación del local* **Decisión de diseño**: `GET /barbers/:barberId/revenue|stats` toma `barberId` explícito en el path (mismo patrón que `AgendaRepository.findScheduleFor`, reutilizado a propósito) — no existe ningún modo "total del local" que este handler entienda; la facturación agregada del local es estructuralmente inalcanzable por esta ruta, no solo rechazada en runtime. Test: `apps/api/test/barber-performance-access.spec.ts`.
- [x] 11.10 GREEN: confirmar el guard de permiso `finance:read:own` vs `finance:read:shop`. Confirmado con actores fixture que reflejan el seed real de la migración 0006 (dueño: `finance:read:shop`/`agenda:read:any`; barbero: `finance:read:own`/`agenda:read:own`) — swap deliberado para probar que no son intercambiables.
- [x] 11.11 RED: barbero marca su propio turno `realizado` y resuelve su propio `sin registrar`. → *barber-profile: Resolución de los turnos propios* `barber-mark-completed.spec.ts` + `barber-confirm-absence.spec.ts`.
- [x] 11.12 RED: barbero intentando resolver el turno de un colega es rechazado. → *barber-profile: Barbero intenta resolver el turno de un colega* Mismos dos archivos — caso de rechazo antes de cualquier escritura (`updateStatusCalls`/`recordCalls` vacíos).
- [x] 11.13 GREEN: acotar `MarkCompletedUseCase`/`ConfirmAbsenceUseCase` con `barberId = actor.barberId` cuando el rol es barbero. **Decisión de diseño**: se agregan `BarberMarkCompletedUseCase`/`BarberConfirmAbsenceUseCase` como clases hermanas de `AdminMarkCompletedUseCase`/`AdminConfirmAbsenceUseCase` (Fase 10) — no se modifican esas dos, mismo patrón ya establecido por `SelfCancelAppointmentUseCase` vs `AdminCancelAppointmentUseCase`: dos políticas de autorización distintas sobre la misma operación de dominio, más claras separadas que fusionadas con un `if`. **Alcance recortado explícitamente**: no se conectó un endpoint HTTP para estas dos clases (el texto de la tarea no lo pide, a diferencia de otras que sí dicen "endpoint"/"formulario"; mismo criterio ya usado en 10.8). Las clases están completas, probadas y listas — cablearlas es una conexión de módulo directa para quien continúe.

**13/13 tareas completas** (11.1-11.13). Requisitos que cierra: **barber-profile** (4/4: Agenda propia filtrada · Estadísticas de cortes propios · Facturación teórica por precio de lista · Resolución de los turnos propios). **Presupuesto**: ~489 líneas de producción (sin tests/fakes) contra el techo de ~300 — 63% sobre presupuesto, consistente con el patrón ya repetido en fases anteriores (3a: 593/400, 8: 504/400, 5a: 556/400); ninguna tarea se recortó para encajar. **Riesgo/limitación de verificación honesta**: en este entorno (Node v24), CUALQUIER módulo que importe `drizzle-orm/postgres-js` (`apps/api/test/*.spec.ts` vía `AppModule`, y todo `packages/infrastructure/**/*.spec.ts`) falla al cargarse con `Cannot require() ES Module ... in a cycle` — un problema de interop ESM/CJS de Node, no de Testcontainers/Docker ni de este código: se reprodujo exactamente el mismo error en specs YA mergeadas de fases anteriores (`day-board.spec.ts`, `authorization.contract.spec.ts`, `day-board.repository.spec.ts`), antes de que Docker entre siquiera en juego. `pnpm typecheck` (7/7 paquetes), `pnpm lint` (0 errores en los 28 archivos de esta fase) y `pnpm depcruise` (0 violaciones) están en verde; los tests nuevos de `apps/api`/Testcontainers quedan reportados como no ejecutados, no como fallidos.
